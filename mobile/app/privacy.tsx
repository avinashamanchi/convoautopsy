...
export function PrivacyCopy() {
  return (
    <View style={styles.copyGroup}>
      ...
      <Text style={styles.copy}>For security and rate limiting, the server HMAC-derives a pseudonymous identifier from the installation token, network signal, and route. Its Durable Object stores only an integer count and window state for a fixed 60-second window; it does not store the raw token, IP address, or message text, and its alarm removes that counter after expiry. Revoking consent removes the local consent and token; it cannot delete data held by Cloudflare or Groq infrastructure.</Text>
